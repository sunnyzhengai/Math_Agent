...
with st.sidebar:
    ...
    if st.button("Reset Progress"):
        save_user_state(username, {"username": username, "skills": {}, "history": []})
...
if st.session_state.current_item:
    ...
    if st.button("Submit"):
        correct, tags, chosen_text = grade(item, choice)
        update_after_answer(state, item["skill_id"], correct, tags)
        save_user_state(username, state)

        # DEBUG: Print the item choices and selected choice
        st.write("DEBUG - Item choices:")
        for c in item["choices"]:
            st.write(f"  ID: {c['id']}, Text: {c['text']}, Tags: {c.get('tags_on_select', [])}")
        st.write(f"DEBUG - You selected: {choice}")
        st.write(f"DEBUG - Correct tags: {tags}")

        # build feedback
        icon = "✅ Correct!" if correct else "❌ Not quite."
